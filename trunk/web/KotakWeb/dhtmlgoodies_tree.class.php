...
************************************************************************************************************/
		...
		var plusNode = 'image/dhtmlgoodies_plus.gif';
		var minusNode = 'image/dhtmlgoodies_minus.gif';
		...
		*/
		...
		function expandAll()
		{
			...
			var image = treeObj.getElementsByTagName('IMG');
			for(var no=0;no<image.length;no++){
				if(image[no].className=='tree_plusminus' && image[no].src.indexOf(plusNode)>=0)expandNode(false,image[no]);
			}
		}
		function collapseAll()
		{
			...
			var image = treeObj.getElementsByTagName('IMG');
			for(var no=0;no<image.length;no++){
				if(image[no].className=='tree_plusminus' && image[no].src.indexOf(minusNode)>=0)expandNode(false,image[no]);
			}
		}
		...
		function initTree()
		{
			...
			for(var no=0;no<image.length;no++){
				if(image[no].className=='tree_plusminus')image[no].onclick = expandNode;
			}
	...
	*/
	...
	function addToArray($id,$name,$parentID,$url="",$target="",$icon="image/dhtmlgoodies_folder.gif", $onclick = ''){
	...
	function addToArrayAss($element){
		...
		$element['icon'] = isset($element['icon']) ? $element['icon'] : 'image/dhtmlgoodies_folder.gif';
	...
	function drawSubNode($parentID){
		if(isset($this->elementArray[$parentID])){
			...
			for($no=0;$no<count($this->elementArray[$parentID]);$no++){
				...
				echo "<img class=\"tree_plusminus\" id=\"plusMinus".$this->elementArray[$parentID][$no]['id']."\" src=\"image/dhtmlgoodies_plus.gif\"><img src=\"".$this->elementArray[$parentID][$no]['icon']."\">";
	...
	function drawTree(){
		...
		for($no=0;$no<count($this->elementArray[0]);$no++){
			...
			echo "<img id=\"plusMinus".$this->elementArray[0][$no]['id']."\" class=\"tree_plusminus\" src=\"image/dhtmlgoodies_plus.gif\">";
